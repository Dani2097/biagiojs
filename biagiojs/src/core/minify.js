...
/**
 * Minificatore CSS "vero" opzionale (lightningcss): collassa shorthand, deduplica
 * selettori/regole, minifica i valori (colori, unità) — byte unici in meno che
 * sopravvivono anche a gzip/Brotli, cosa che la sola rimozione di whitespace non fa.
 * Se lightningcss non è installato ripiega su `minifyCss` (regex, zero dipendenze).
 * Ritorna una funzione SINCRONA (css) => css minificato, usabile nei replace callback.
 */
export async function loadCssMinifier() {
  try {
    const { transform } = await import('lightningcss');
    return css => {
      try {
        const { code } = transform({ filename: 'inline.css', code: Buffer.from(css), minify: true });
        return code.toString();
      } catch { return minifyCss(css); }   // CSS non standard nel blocco: fallback conservativo
    };
  } catch {
    return minifyCss;
  }
...
export async function optimizeHtml(html, { purge = true, minify = true, scripts = true } = {}) {
  ...
  if (purge || minify) {
    ...
    const templates = [];
    html = html.replace(/<template\b[\s\S]*?<\/template>/gi, m => {
      templates.push(m);
      return `\x00T${templates.length - 1}\x00`;
    });

    html = html.replace(/<style(\s[^>]*)?>([\s\S]*?)<\/style>/gi, (full, attrs = '', css) => {
      const noPurge = !purge || /data-cvw-no-purge/i.test(attrs || full);
      let out = css;
      if (purge && !noPurge) out = purgeCss(out, html);
      if (minify) out = cssMin(out);
      const attr = attrs || '';
      return out.trim() ? `<style${attr}>${out}</style>` : '';
    });

    html = html.replace(/\x00T(\d+)\x00/g, (_, n) => templates[+n]);
  }
