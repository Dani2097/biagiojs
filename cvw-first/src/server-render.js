/**
 * CVW-First — rendering per-request condiviso (dev Vite + adapter Node).
...
export function matchRoute(pagesDir, url) {
  ...
  for (const file of walkPages(pagesDir)) {
    const routeSegs = file.replace(/\.page\.(js|ts|cvw)$/, '').split('/');
...
export async function renderRequest(root, url, { loadModule, request = {}, userId = 'ssr', dev = false, meta = {} } = {}) {
  ...
  const site = existsSync(configPath) ? (await load(configPath)).default.site : { name: 'CVW Site', baseUrl: 'https://example.com' };
  ...
  const mod = match.file.endsWith('.cvw')
    ? loadCvwFile(join(root, 'pages', match.file))
  ...
  return renderPage(graph, { title: page.title, head, overlay, site, page, assets, experiments, thresholds });
}
