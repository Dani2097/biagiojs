/**
 * CVW-First — compiler per la sintassi single-file `.page.cvw` (paper §3).
 *
...
/** Compila il sorgente .cvw in una definizione di pagina { default } usabile dal CLI. */
export function compileCvw(source, filename = 'anonymous.cvw') {
  // <page ... /> — metadati pagina
  const pageMatch = source.match(/<page\s+([\s\S]*?)\/>/);
  if (!pageMatch) throw new Error(`[cvw compiler] ${filename}: manca il tag <page ... />`);
  ...
  for (const m of source.matchAll(/<component\s+([^>]*)>([\s\S]*?)<\/component>/g)) {
    ...
    const body = m[2];
    if (!attrs.id) throw new Error(`[cvw compiler] ${filename}: component senza id`);

    const tpl = body.match(/<template>([\s\S]*?)<\/template>/);
    if (!tpl) throw new Error(`[cvw compiler] ${filename}: component "${attrs.id}" senza <template>`);
    ...
    if (hyd) {
      ...
      catch (e) { throw new Error(`[cvw compiler] ${filename}: script hydrate di "${attrs.id}" non valido: ${e.message}`); }
    }
  ...
  if (!components.length) throw new Error(`[cvw compiler] ${filename}: nessun <component>`);
...
/** Carica e compila un file .cvw dal disco. */
export function loadCvwFile(path) {
  return compileCvw(readFileSync(path, 'utf8'), path);
}
